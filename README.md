...
It also supports Brotli and Gzip precompressed files and allows for customization of response headers. Additionally, it comes with built-in instrumentation for [OpenTelemetry](https://opentelemetry.io/) and [Prometheus](https://prometheus.io/).

## Features & Purpose in Polyfea Ecosystem

`spa_base` serves as a standardized hosting unit for Polyfea microfrontends. It abstracts away the complexity of configuring a web server (like Nginx or Apache) for modern JavaScript applications, allowing teams to focus on building features.

*   **Zero-Config SPA Routing**: Automatically handles 404s by falling back to `index.html`, enabling client-side routing (HTML5 History and Navigation API) out of the box.
*   **Module Resolution for Microfrontends**: Aids in loading ES modules by automatically resolving requests without extensions to `.js`, `.mjs`, or `.cjs` files. This bridges the gap between development environments and production execution for import-map based architectures.
*   **Pre-compressed Content Delivery**: Detects and serves side-by-side compressed assets (`.br`, `.gz`) generated by build tools, ensuring optimal load times without on-the-fly compression overhead.
*   **Context Awareness**: The `base-url` configuration allows microfrontends to be hosted on sub-paths easily, crucial for composing multiple apps under one domain.
*   **Observability**: Native OpenTelemetry integration ensures that frontend serving infrastructure is as observable as backend services.
...
# file with the same name and a .br extension, and the client accepts br encodings. 
# Set this option to true to  disable servinf of Brotli compressed side files.
#
# To generate the required Brotli files, you can use precompress tool with command
# precompress -t gz,br -e **.gz,**.br,**.map,**.woff2 dist 
# 
...
# file with the same name and a .gz extension, and client accepts gzip compression. 
# Set this option to true to disable serving of Gzip-ed side files.
#
...
# The default behavior is to initialize them using 'none' exporters.
telemetry-disabled: false

# Regular Expression for Import Fallbacks (Default: empty)
# If the file is not found then the service tries to add one of ".mjs", ".js", ".cjs" 
# extensions to yhe path and search for the file. This is used to simplify usage 
# of impartmap path prefixes served by the service itself in the cases where client's 
# bundler does not add such extensions automatically. FOr example in Vite if you set 
# lit as external and uses `import { property, state } from 'lit/decorators'` instead of
# the correct `import { property, state } from 'lit/decorators.js';` 
# then it will fail in runtime if Import Fallbacks are disabled. 
#
# If this setting is empty then all files are tried for the mentioned extensions. If set 
# to "disable" then no fallbacks are tested. If this is set to valid regular expression 
# then only files with matching the resource path will be tried. uested path.
# 
# See also # [importmap prefixes](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/script/type/importmap#mapping_path_prefixes) on MDN
...
| SPA_BASE_IMPORT_FALLBACK_REGEX   |            | Regular Expression for Import Fallbacks (Default: empty). If set to a valid regular expression, only requests with paths matching this expression will attempt import fallbacks when a resource is not found. If empty, all paths are attempted for import fallbacks.  Setting it to "disable" will disable import fallbacks entirely.  The service will try to add ".js/mjs/cjs" extensions to the requested path. |
